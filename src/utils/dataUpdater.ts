...
class GoTokyoParser {
  static parseMonthData(html: string): Partial<MonthConfig> {
    ...
    const parsed: Partial<MonthConfig> = {
      ...
      // 更新外部数据源信息
      external_sources: {
        gotokyo_url: this.extractSourceUrl(html),
...
export class DataUpdater {
  ...
  static async updateMonthConfig(monthId: number): Promise<MonthConfig | null> {
    ...
    const updatedConfig: MonthConfig = {
      ...
      // 更新外部数据源信息
      external_sources: {
        ...baseConfig.external_sources,
        ...externalData.external_sources
      }
    };
  ...
  static getUpdateStatus(monthId: number): {
    ...
    const lastUpdated = config?.external_sources?.last_updated || null;
    const hasExternalSource = !!config?.external_sources?.gotokyo_url;
